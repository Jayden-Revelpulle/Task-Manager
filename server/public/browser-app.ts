...
const loadingDOM = document.querySelector('.loading-text') as HTMLDivElement;
...
const formAlertDOM = document.querySelector('.task-form .form-alert') as HTMLDivElement;
...
const showTasks = async (): Promise<void> => {
    ...
    try {
        const {
            data: { tasks },
        } = await axios.get<{ tasks: Task[] }>('/api/v1/tasks');
